...
The athermal stress is of Taylor form, $\tau_a=\alpha\mu b\,s_c(\sqrt{\rho_f}+\Lambda_b)$, where $\rho_f=\beta\rho$ is the forest part of the total density $\rho$ (with $\rho_m=f_m\rho_f$), $\Lambda_b$ is the inverse spacing of boundaries, and $s_c$ is a solute strengthening factor defined below. The dislocation density obeys Kocks--Mecking kinetics~\cite{MeckingKocks} with an additional diffusion-controlled annihilation term,
\begin{equation}
\frac{d\rho}{d\gamma}=k_1\big(\sqrt\rho+\Lambda_b\big)-k_2\rho-\frac{K\,D(T)}{\gd}\,\rho^2 ,
\label{eq:km}
\end{equation}
which converts the time-based climb annihilation rate $K D\rho^2$ into a strain increment. At steady state the density is the unique positive root of the right-hand side. When the last term dominates, $\tau_a\propto(\gd/D)^{1/3}$, which is the natural creep law of dislocation networks. Solute atmospheres form on segments arrested at forest junctions during the waiting time $t_a=\rho_m b\lambda_f/\gd$, with $\lambda_f=\rho_f^{-1/2}$~\cite{CottrellBilby,Louat1981,Cheng2001},
\begin{equation}
\frac{C}{C_0}=1+\sum_iC_i\left\{1-\exp\!\left[-\left(t_a\nu_ie^{-Q_i/\kT}\right)^{\alpha_i}\right]\right\},
\label{eq:aging}
\end{equation}
and increase the junction strength by $s_c=\sqrt{C/C_0}$.

Particles are bypassed either by thermally activated detachment at a rate $\nu_{\rm th}=\nu_0\exp\{-F[1-(\tau_p/\hat\tau)^{p}]^{q}/\kT\}$ or by glide-assisted local climb at a rate $\nu_{\rm cl}=v_c/h_{\rm eff}$, with $v_c\propto D\,[\exp(\tau_p\Omega/\kT)-1]$ and a climb height $h_{\rm eff}=h(1-\tau_p/\hat\tau)$ that shrinks as the line is pressed against the particle~\cite{BrownHam,RoslerArzt}. The two routes are alternatives, so their rates add, and the particle stress $\tau_p$ follows from $\gd=\rho_m b\lambda_p(\nu_{\rm th}+\nu_{\rm cl})$, where $\lambda_p$ is the particle spacing. Diffusional (Nabarro--Herring and Coble) flow transports matter rather than dislocations and therefore adds its rate to that of Eq.~(\ref{eq:flow}).
