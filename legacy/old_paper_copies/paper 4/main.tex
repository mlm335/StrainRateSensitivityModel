...
Here we construct such a rule and show that it can be built up hierarchically. Its minimal form, kink-pair mobility acting against a forest, describes W. Adding solute aging and diffusion-controlled recovery of the same forest describes Zr, including its negative rate sensitivity. Adding grain-boundary storage, particle pinning with thermal and climb bypass, and diffusional flow describes Eurofer-97 in tension and in creep. Each level contains the previous one unchanged (Fig.~\ref{fig:scheme}).
...
\begin{tikzpicture}[font=\footnotesize,>=Stealth,
  ...
  \node[lvl,fill=red!7,text width=6.55cm,below=0.2cm of I.south west,anchor=north west] (III) {\textbf{III} (Eurofer-97)\quad boundary storage $\Lambda_b$;\\ particle pinning $\tau_p(\gd,T)$: detachment \emph{or} local climb (rates add);\\ diffusional flow as an independent carrier (rates add)};
...
% ---------------------------------------------------------------------
The flow rule is the Orowan relation~\cite{Orowan1940} for the mobile screw population together with a superposition of resistances~\cite{Seeger,KocksArgonAshby},
\begin{equation}
\gd=\rho_m b\,v_g(\ts),\qquad \tau=\tau_a(\bm s)+\ts+\sum_g\tau_g(\gd,T),
\label{eq:flow}
\end{equation}
where $\ts$ is the effective stress on the kink-pair process, $\tau_a$ is the long-range athermal stress fixed by the microstructural state $\bm s$, and $\tau_g$ are the stresses carried by sparse, strong pinning populations that act on the line simultaneously with lattice friction, as in Kocks' superposition of dense-weak and sparse-strong obstacles~\cite{KocksArgonAshby}. The glide velocity is the kink-pair law of Ref.~\cite{Po2016}, which combines the $(p,q)$ barrier profile of Kocks, Argon and Ashby~\cite{KocksArgonAshby} with kink-pair nucleation in the long-segment limit~\cite{DornRajnak1964,HirthLothe},
\begin{equation}
v_g=\frac{\ts b}{B_d}\exp\!\left\{-\frac{\Delta H_0}{2\kT}\left[\left(1-\left(\frac{\ts}{\tau_P}\right)^{p}\right)^{q}-\frac{T}{T_0}\right]\right\},
\label{eq:vg}
\end{equation}
in which $B_d$ is the kink drag coefficient, $\tau_P$ is the Peierls stress, the factor $1/2$ reflects long screw segments, and $T/T_0$ acts as an entropic correction to the barrier.

The athermal stress is of Taylor form~\cite{Taylor1934,KocksMecking2003}, $\tau_a=\alpha\mu b\,s_c(\sqrt{\rho_f}+\Lambda_b)$, where $\rho_f=\beta\rho$ is the forest part of the total density $\rho$ (with $\rho_m=f_m\rho_f$), $\Lambda_b$ is the inverse spacing of boundaries, and $s_c$ is a solute strengthening factor defined below. The dislocation density obeys Kocks--Mecking kinetics~\cite{Kocks1976,MeckingKocks,KocksMecking2003}, in which $k_1\sqrt\rho$ is athermal storage at forest obstacles, $k_1\Lambda_b$ is storage at boundaries~\cite{EstrinMecking1984,Estrin1996}, and $k_2\rho$ is dynamic recovery by cross slip and glide annihilation, supplemented by climb-controlled (static) recovery~\cite{SandstromLagneborg1975,Sandstrom1977,Nes1995,SandstromHallgren2012},
\begin{equation}
\frac{d\rho}{d\gamma}=k_1\big(\sqrt\rho+\Lambda_b\big)-k_2\rho-\frac{K\,D(T)}{\gd}\,\rho^2 ,
\label{eq:km}
\end{equation}
The last term is the time-controlled annihilation rate $d\rho/dt=-KD\rho^2$ of dislocation dipoles and networks by climb~\cite{SandstromLagneborg1975,Sandstrom1977,Nes1995} divided by $\gd$; unlike $k_2\rho$, which acts per unit strain, it operates whether or not the material deforms, and its relative weight grows as $D/\gd$ increases. At steady state the density is the unique positive root of the right-hand side. When the last term dominates, $\tau_a\propto(\gd/D)^{1/3}$, which is the natural creep law of dislocation networks~\cite{Poirier1985,Blum2002}. Solute atmospheres form on segments arrested at forest junctions during the waiting time $t_a=\rho_m b\lambda_f/\gd$, with $\lambda_f=\rho_f^{-1/2}$~\cite{McCormick1988,KubinEstrin1990}; the atmosphere grows with the Cottrell--Bilby kinetics~\cite{CottrellBilby} and saturates as described by Louat~\cite{Louat1981},
\begin{equation}
\frac{C}{C_0}=1+\sum_iC_i\left\{1-\exp\!\left[-\left(t_a\nu_ie^{-Q_i/\kT}\right)^{\alpha_i}\right]\right\},
\label{eq:aging}
\end{equation}
and increase the junction strength by $s_c=\sqrt{C/C_0}$~\cite{Cheng2001,MulfordKocks1979}.

Particles are bypassed either by thermally activated detachment at a rate $\nu_{\rm th}=\nu_0\exp\{-F[1-(\tau_p/\hat\tau)^{p}]^{q}/\kT\}$ ~\cite{KocksArgonAshby,RoslerArzt} or by glide-assisted local climb at a rate $\nu_{\rm cl}=v_c/h_{\rm eff}$, with the climb velocity $v_c\propto D\,[\exp(\tau_p\Omega/\kT)-1]$~\cite{HirthLothe} and a climb height $h_{\rm eff}=h(1-\tau_p/\hat\tau)$ that shrinks as the line is pressed against the particle~\cite{BrownHam,ArztAshby1982}. The two routes are alternatives, so their rates add, and the particle stress $\tau_p$ follows from $\gd=\rho_m b\lambda_p(\nu_{\rm th}+\nu_{\rm cl})$, where $\lambda_p$ is the particle spacing. Diffusional flow~\cite{Nabarro1948,Herring1950,Coble1963,FrostAshby} transports matter rather than dislocations and therefore adds its rate to that of Eq.~(\ref{eq:flow}).
